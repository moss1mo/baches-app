...
.controller('ReportIncidentMapCtrl', function($ionicPlatform, $scope, $state, $q, $ionicActionSheet,
    ...
    if (selectedCluster.status == 0 || selectedCluster.status == 1) {
      ret.push({
        ...
        onTap: function(e) {
          $timeout(function() {
            $ionicPopup.show({
              template: '¿Está seguro de marcar éste reporte como Denegado?',
              title: 'Confirmación',
              buttons: [
                {
                              ...
                              },
                              {
                                ...
                                onTap: function(e) {
                                  ...
                                  if (motivoScope.params.motivo == undefined) {
                                    e.preventDefault();
                                    $timeout(function() {
                                      $cordovaToast.showLongCenter('Favor de especificar el motivo de porque denega el reporte');
                                    });
                                  } else {

                                    $ionicLoading.show({
                          template: 'Marcando reporte como Denegado ...'
                        });
                            ...
                            motivo: motivoScope.params.motivo}).then(function(online) {
                              ...
                              if (online) {
                                $timeout(function() {
                                  $cordovaToast.showLongCenter('Bache marcado como Denegado en servidor');
                                });
                              } else {
                                $timeout(function() {
                                  $cordovaToast.showLongCenter('Bache marcado como Denegado en dispositivo');
                                });
                              }
                           }, function(err) {
                          if (err.status == 401) {
                            ...
                            $timeout(function() {
                              $cordovaToast.showShortCenter('Error marcando bache como Denegado: ' + errToStr(err));
                            });
                          }
                        }).finally(function() {
                          $ionicLoading.hide();
      ...
      ret.push({
        ...
        onTap: function(e) {
          $timeout(function() {
            $ionicPopup.show({
              template: '¿Está seguro que desea cambiar el estatus de éste reporte a Reparado? Esta acción no se podrá deshacer.',
              title: 'Confirmación',
              buttons: [
                {
                  text: '<i class="icon ion-close dark"></i>',
                              ...
                              },
                              {
                                ...
                                onTap: function(e) {
                                  ...
                                  if (materialTypeScope.params.clusterArea == undefined) {
                                    e.preventDefault();
                                    $timeout(function() {
                                      $cordovaToast.showLongCenter('Favor de especificar la medida del bache');
                                    });
                                  } else {
                                    $ionicLoading.show({
  ...
  var connectionStatus = false;
    setInterval(function () {
        ...
        if(navigator.onLine){
          sendAutomaticOffline();
        }
    }, 10000);


   function sendAutomaticOffline(){
        ...
        showOfflineDataScope.offlineChangeIncidentStatuses.length) > 0) {
            return incidentSvc.submitOfflineData(
              ...
              if (uploadCount >= (showOfflineDataScope.offlineSubmitIncidents.length + showOfflineDataScope.offlineChangeIncidentStatuses.length)) {
                $scope.reloadClusters();
                $timeout(function() {
                  $cordovaToast.showLongCenter('Todos los datos offline han sido guardados de forma automática');
                });
              } else {
                if (uploadCount > 0) {
                  $scope.reloadClusters();
                  $timeout(function() {
                    $cordovaToast.showLongCenter('Algunos datos offline han sido guardados de forma automática');
                  });
                } 
              }
            }, function(err) {
              $timeout(function() {
                $cordovaToast.showShortCenter('Error guardando los datos de forma automática: ' + errToStr(err));
              });
            }).finally(function() {
              $ionicLoading.hide();
            });
  ...
  {
          $ionicPopup.show({
              template: '¿Desea capturar bache sin reparar?',
              title: 'Confirmación',
              buttons: [
                {
                  text: '<i class="icon ion-close dark"></i>',
                ...
                },
                {
                  ...
                  onTap: function(e) {
                    ...
                    incidentSvc.takePicture().then(function(photoLocAnt) {
                      if (photoLocAnt == cordovaplConfig.DEFAULT_PHOTO) {
                        $timeout(function() {
                          $cordovaToast.showLongCenter("Es obligatorio incluir foto en el reporte");
                        });
                      }
                    ...
                    $ionicPopup.show({
              template: '¿Desea capturar bache reparado?',
              title: 'Confirmación',
              buttons: [
                {
                  text: '<i class="icon ion-close dark"></i>',
                ...
                },
                {
                                ...
                                onTap: function(e) {
                                  ...
                                  if (materialTypeScope.params.clusterArea == undefined) {
                                    ...
                                    $ionicLoading.show({
                                     ...
                                     $scope.reporteIncidente.photo = photoLocAnt;
                                     $scope.reporteIncidente.photo_repaired = photoLoc;
                                    ...
                                    $scope.submitIncidentCanceller = $q.defer();
                                    $scope.reporteIncidente.userRole = $scope.userDetail.role;
                                    $scope.reporteIncidente.materialTypeId = materialTypeScope.materialTypes[0].id;
                                    $scope.reporteIncidente.clusterArea = materialTypeScope.params.clusterArea;
                                    $scope.reporteIncidente.shareSocnets = [];



                                    $ionicLoading.show({
                                      template: 'Enviando reporte de bache reparado al servidor...'
                                    });
                                    incidentSvc.submitIncident($scope.reporteIncidente).then(function(reporteIncidente) {
                                      ...
                                      if (reporteIncidente.online) {
                                        if (reporteIncidente.fb_failure_mode == 1) {
                                          ...
                                          $timeout(function() {
                                            $cordovaToast.showLongCenter('Reporte de bache reparado ha sido guardado en servidor. Link con facebook desactivado.');
                                          });
                                        } else {
                                          $timeout(function() {
                                            $cordovaToast.showLongCenter('Reporte de bache reparado enviado exitosamente.');
                                          });
                                        }
                                      } else {
                                        $timeout(function() {
                                          $cordovaToast.showLongCenter('Reporte de bache reparado ha sido guardado en dispositivo');
                                        });
                                      }
                  ...
                  }//cierra segundo btn de accion positiva
                }
            ]});
    }
...
.controller('ReportIncidentSuccessCtrl', function($scope, $state, $q, $ionicHistory,
  ...
  $scope.newReportarBache = function() {
    $state.go('tab.report-incident.map').then(function() {
      scrollToSection('reportar_supervisor');
    });
  }
    ...
    if ($scope.loginFormData.loginName == undefined) {
      ...
      return user.loginCiudadano($scope.loginFormData.loginName, $scope.loginFormData.password).then(function(authData) {
        ...
        if (err.status == 0) {
          ...
          /** Se comenta por el momento el registro
